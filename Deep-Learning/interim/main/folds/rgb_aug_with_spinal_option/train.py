...
dataset_train = GenericDataset(split="train", fold_name="folds/fold_5_train.txt", path=DATASET_PATH)
train_loader = DataLoader(dataset_train, batch_size=BATCH_SIZE, unsupervised=True)

dataset_val = GenericDataset(split="val", fold_name="folds/fold_5_val.txt", path=DATASET_PATH)
val_loader = DataLoader(dataset_val, batch_size=BATCH_SIZE, unsupervised=True)

experiment_name = prepare_experiment(experiment_name="label_aug_w_spinal_fold5")
