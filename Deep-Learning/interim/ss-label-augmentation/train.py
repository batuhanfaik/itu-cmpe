...
train_loader = torch.utils.data.DataLoader(
    DataReader(mode='train', fold_name="folds/fold_3_train.txt", path=DATASET_PATH),
    batch_size=BATCH_SIZE, shuffle=True, num_workers=num_workers, drop_last=True)
"""
dataset_train = GenericDataset(split="train", fold_name="folds/fold_1_train.txt", path=DATASET_PATH)
train_loader = DataLoader(dataset_train, batch_size=BATCH_SIZE, unsupervised=True)

dataset_val = GenericDataset(split="val", fold_name="folds/fold_1_val.txt", path=DATASET_PATH)
val_loader = DataLoader(dataset_val, batch_size=BATCH_SIZE, unsupervised=True)
#val_loader = torch.utils.data.DataLoader(
#    DataReader(mode='val', fold_name="folds/fold_3_val.txt", path=DATASET_PATH),
#    batch_size=BATCH_SIZE, shuffle=True, num_workers=num_workers, drop_last=True)
