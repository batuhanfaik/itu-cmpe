...
void WorkPlan::checkAvailableNextTimesFor(Task *delayed) {
    ...
    int last_available_time = 16;
    bool task_is_delayed = false;
    Task *day_holder = head;
    // Find the first task of current day
    while (day_holder->day != delayed->day) {
        day_holder = day_holder->next;
    }
    Task *last_task = day_holder;
    // Find the last task of current day
    while (last_task){
        last_available_time = last_task->time;
    ...
    while (available_day <= last_day && !task_is_delayed) {
        while (available_time >= 8 && available_time <= last_available_time && !task_is_delayed) {
            ...
            if (getTask(available_day, available_time)) {
                ...
                if (available_time == 8) {   // Insert as first task of the day
                    ...
                    Task *insert_before = NULL;
                    while (first_time_of_day <= last_available_time and !insert_before) {     // Coding as failsafe as possible
                ...
                } else if (available_time <= last_available_time) {   // Insert as counterpart of an existing task
